...
$
En introduisant les symboles de Christoffel $Gamma^beta""_(mu nu) = 1/2 g^(beta alpha) (g_(mu alpha, nu) + g_(nu alpha, mu) - g_(mu nu, alpha) )$ on a :
$
...
$
On trouve que : $Gamma^(mu)""_(alpha beta) = croh(l: mu, m: alpha, n: beta) = (diff x^mu)/(diff xi^nu) (diff^2 xi^nu)/(diff x^alpha diff x^beta)$
...
Ainsi $ Gamma^lambda""_(mu nu) ' = (diff x'^lambda)/(diff x^rho) croh(m: sigma, l: rho, n: alpha) (diff x^sigma)/(diff x'^mu) (diff x^alpha)/(diff x'^nu)
+ (diff x'^lambda)/(diff x^alpha) (diff^2 x^alpha)/( diff x'^mu diff x'^nu) \ $ <pasbontenseur>

Donc d’après @pasbontenseur, $Gamma^alpha""_(mu nu)$ n’est pas un tenseur
...
=== Commuteur des Co-dérivés covariante & Tenseur de Riemann
...
$
Ainsi, on reconnait le tenseur de Riemann définis par :
$
  R^sigma""_(mu nu kappa) = diff_nu Gamma^sigma""_(mu kappa) - diff_kappa Gamma^sigma""_(mu nu) + Gamma^sigma""_(nu lambda) Gamma^lambda""_(mu kappa) - Gamma^sigma""_(kappa lambda) Gamma^lambda""_(mu nu)
$
Donc on obtient la relation suivante :
$
  [nabla_mu; nabla_nu]v^rho = R^rho""_(alpha mu nu) v^alpha
$

On définis également les deux contraction du tenseur de Riemann suivant :
- Le tenseur de Ricci :
$
...
==== Calcule des Christoffel<calcristo>
...
$
Où on note les dérivés par un point les dérivés par à $tau$ et $B'(r) et A'(r)$ les dérivés par rapport à $r$.\
...
$
Ainsi pour un objet tombant d’un point distant ($r = R_0$), alors il atteindra la singularité ($r = 0$) en :
$
...
On remarque, comme attendue, que ici ce sont les rayons sortants qui sont continue pour $r = R_s$. De même, si l’on regarde la structure des
cônes de lumière : cecis sont entièrement #text(style: "italic")[tourné] vers l’exterieur pour $r < R_s$, ainsi un rayon lumineux seras forcément éjécté de la région $r < R_s$.\
Le rayon $R_s$ forme aussi une frontière, également, appelée #text(style: "italic")[horizon des évènements], et ainsi, au contraire de la solution trou noir,
un photon émis en $r < R_s$ atteind forcément un observateur lointain, l’objet nous apprait alors blanc.\
Un tel objet est appelée #text(style: "italic")[trou blanc]
...
Pour le calcule des symboles de Christoffel de la @calcristo, on à faits un programme, ici faits via le CAS : #link("https://xcas.univ-grenoble-alpes.fr/index.html", [giac/Xcas]).\

Pour executer ce programme il faut s’éléctioner dans `cfg -> Mode (syntax) -> Xcas`
