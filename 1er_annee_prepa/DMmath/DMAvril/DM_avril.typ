...
#set heading(numbering: (..numbers) => {
  ...
  if n == 1 { question.update(1) } else if n == 2 {
    [Partie ]
    ...
    question.display(na)
    "."
  } else if n == 4 { numbering("a.", numbers.pos().at(3) + 1) } else if n == 5 {
    numbering("i.", numbers.pos().at(4) + 1)
  }
})
...
#t(1) Or comme $per v2 so [|ur; n|], v1_v2 so QQ$ donc $ber v3, com_ur, dots.c, com_n so NN,per v2 so [|ur; n|], v1_v2 ing com_v2/v3$. Ainsi
...
#t(
  1,
) #undermath[Donc *L* est une extensions finis de $k$ et $[upright(bold(L)) : k] ing [upright(bold(L)) : upright(bold(K))] [upright(bold(K)) : k]$]
...
#t(
  1,
)#undermath[on a bien que $KK[alpha]$ est un sous-corps de $LL <=> alpha^(al ur) so KK[alpha] <=> alpha$ est algébrique sur $KK$ ]\
...
#t(
  1,
) #undermath[Ainsi $pol1 ing com X^#na(4) al #na(10) com X^#na(2) ti com$, ainsi on prouve que $alpha$ est algébrique] \
...
#t(
  1,
) #undermath[Et donc $QQ[sqrt(#na(2)), sqrt(#na(3))] ing {v1 ti v2 sqrt(tur) ti com sqrt(an) ti v3 sqrt(geb), v1,v2, com, v3 so QQ}$]\
